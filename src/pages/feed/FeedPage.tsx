...
import { Heart, MessageCircle, Share2, RefreshCw, Plus } from 'lucide-react';
...
const FeedPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Stories — иконки пользователей с новыми историями */}
      <div className="border-b border-border py-3 px-2 overflow-x-auto scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
      ...
      <div className="py-4 px-2 space-y-4">
        {posts.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">
            Пока нет постов. Создайте первый в разделе «Создать».
          </p>
        ) : (
          posts.map((post) => {
      ...
      <SharePostSheet
        ...
        post={postToShare}
        author={postToShare ? getFeedUser(postToShare.authorId) : undefined}
      />
    </div>
  );
};
